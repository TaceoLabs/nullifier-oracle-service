...
/// Holds the active OPRF sessions with multiple peers.
...
async fn init_session<Auth: Clone + Serialize + DeserializeOwned + Send + Sync + 'static>(
    service: String,
...
///   `init_sessions` call.
/// - The order of the peers matters: we return responses in the order provided and they need to match the original session list. This is crucial because Lagrange coefficients are computed in the meantime, and they need to match the shares obtained earlier.
///
...
/// Initializes new OPRF sessions by opening a web-socket at `/api/v1/oprf` on a list of peers, collecting responses until the given `threshold` is met.
///
/// Peers are queried concurrently. Errors from some services are logged and ignored, unless they prevent reaching the threshold.
///
