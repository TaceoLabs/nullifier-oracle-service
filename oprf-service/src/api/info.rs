...
//! Currently WIP.
//!
//! Shall provide information about this node, e.g. version, git SHA, ..
//!
...
/// Stub that responds with a response to identify whether node operators upgraded successfully or not
///
/// Returns `200 OK` with a string response.
async fn info(State(_app_state): State<AppState>) -> Result<impl IntoResponse, ApiError> {
    Ok((StatusCode::OK, "node_operator_test_upgrade_1"))
}
