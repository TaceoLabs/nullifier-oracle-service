...
use crate::{
    ...
    services::chain_watcher::{
        ChainEpoch, ChainWatcher, ChainWatcherError, ChainWatcherService, MerkleRoot,
    },
};

type MerkleRootStore = HashMap<ChainEpoch, MerkleRoot>;

struct HttpMockWatcher {
    config: Arc<OprfConfig>,
    chain_url: String,
    latest_epoch: ChainEpoch,
...
impl HttpMockWatcher {
    fn get_merkle_root_by_epoch_inner(&self, epoch: ChainEpoch) -> Option<MerkleRoot> {
