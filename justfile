...
run-rp-registry:
    cargo build --workspace --release
    mkdir -p keys
    ./target/release/test-setup-helper --private-keys-out-dir ./keys --chain-ws-rpc-url ws://127.0.0.1:8545
...
run-services:
    ...
    cargo build --workspace --release
    # anvil wallet 7
    RUST_LOG="oprf_service=trace,warn" ./target/release/oprf-service --user-verification-key-path ./circom/query.vk.json --bind-addr 127.0.0.1:10000 --private-key $(cat keys/oprf_peer_private_key_0) --environment dev --wallet-private-key 0x4bbbf85ce3377467afe5d46f804f221813b2bb87f24d81f60f1fcdbf7cbf4356 --key-gen-zkey-path ./circom/keygen_13.zkey --key-gen-witness-graph-path ./circom/keygen_graph.bin > logs/service0.log 2>&1 &
    pid0=$!
    echo "started service0 with PID $pid0"
    # anvil wallet 8
    RUST_LOG="oprf_service=trace,warn" ./target/release/oprf-service --user-verification-key-path ./circom/query.vk.json --bind-addr 127.0.0.1:10001 --private-key $(cat keys/oprf_peer_private_key_1) --environment dev --wallet-private-key 0xdbda1821b80551c9d65939329250298aa3472ba22feea921c0cf5d620ea67b97 --key-gen-zkey-path ./circom/keygen_13.zkey --key-gen-witness-graph-path ./circom/keygen_graph.bin > logs/service1.log 2>&1 &
    pid1=$!
    echo "started service1 with PID $pid1"
    # anvil wallet 9
    RUST_LOG="oprf_service=trace,warn" ./target/release/oprf-service --user-verification-key-path ./circom/query.vk.json --bind-addr 127.0.0.1:10002 --private-key $(cat keys/oprf_peer_private_key_2) --environment dev --wallet-private-key 0x2a871d0798f97d79848a013d4936a73bf4cc922c825d33c1cf7073dff6d409c6 --key-gen-zkey-path ./circom/keygen_13.zkey --key-gen-witness-graph-path ./circom/keygen_graph.bin > logs/service2.log 2>&1  &
    pid2=$!
