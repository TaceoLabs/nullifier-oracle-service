...
use alloy::{
    ...
    signers::{
        k256::ecdsa::{SigningKey, signature::Signer as _},
...
pub struct OprfDevClientConfig {
    ...
    #[clap(
        ...
        default_value = "0x2de8bC004E3141451B50F076C67bB5a4C31DD846"
    )]
...
async fn run_nullifier(
    ...
    let rp_request = world_id_core::types::RpRequest {
        ...
        oprf_public_key: world_id_core::oprf_types::crypto::OprfPublicKey::new(
    ...
    let (_proof, _nullifier) = authenticator
        .generate_proof(signal_hash.into(), rp_request, credential)
        .await
...
fn prepare_nullifier_stress_test_oprf_request(
    ...
    let primitives_rp_id =
        world_id_primitives::rp::RpId::new(oprf_key_id.into_inner().try_into().expect("fits"));
    let primitives_rp_nullifier_key =
        world_id_primitives::rp::RpNullifierKey::new(oprf_public_key.inner());
